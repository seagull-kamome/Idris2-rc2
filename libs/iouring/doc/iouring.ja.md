...
- **`URing.pending`/`pendingAddrs`は、進行中のバッファ/sockaddrを、それぞれの
  completion(完了)が消費されるまで生かし続けます。** このパッケージを書いている間に
  ...
  (SortedMap Bits64 Buffer)`、sockaddr用の`pendingAddrs : IORef
  (SortedMap Bits64 AnyPtr)`)へ格納するようになりました——呼び出し元自身の
  コードがその後どう振る舞おうと生き残る、ライブラリ側が保持する2つ目の
  参照です(生きた`Buffer`参照、あるいは文字通り`free`を先送りする形)。
  ((1)の以前の壊れた設計にも、実は使える回避策はありました——`buf`への
  何らかの後続の参照を自分で保持しておけば、rc2の通常の「最初の出現で
  move、以降の出現はdup」という所有権規則がそれを守ってくれます——しかし
  それは呼び出し側が覚えておかなければならない、忘れやすい落とし穴であり、
  このパッケージが黙って頼ってよいものではありません。(2)にはそもそも
  同等の呼び出し側での回避策すらありません。生の`malloc`済みポインタは、
  そもそもrc2の参照カウント対象ではないからです)。

  最初のバージョンでは、それぞれのマップはキーを持たない単なる`List`
  でした——正しくはあるものの、どのエントリも`URing`全体が片付く(`exit`)
  まで一切回収されず、長時間動き続ける呼び出し側(`exit`を呼ぶのはシャット
  ダウン時だけで、その間ずっとread/write/send/recv/connectを発行し続ける
  サーバなど)では、両方のリストが増え続けるだけでした。これを直すため、
  各マップのキーを`prepRead`/`prepWrite`/`prepSend`/`prepRecv`/
  `prepConnect`が新たに要求する必須引数`userData`そのものにしました
  (他の`prep*`と違い、後から`setUserData`で付け直すのではありません——
  挿入する時点でこのキーが確定している必要があるからです)。そして
  `waitCompletion`/`pollCompletion`が、対応する`Completion`を読み戻した
  その瞬間にエントリを解放するようにしました。`exit`は、一度も
  completionを消費されなかったものだけを片付ける最後の保険として残って
  います。この変更が持ち込んだ注意点が1つあります: 同じリング上でこの5関数
  のうち2つを*同時に*発行するときは、`userData`を必ず別々の値にしなければ
  なりません——このパッケージの他の箇所でタグが重複しても「completionを
  見分けにくくなる」だけで済むのに対し、ここでの重複は後から呼んだ側の
  挿入が先の側のマップエントリを黙って上書きしてしまい、カーネルがまだ
  触っているかもしれない最初の`Buffer`/`sockaddr`への保護用参照を失う
  ——このしくみ自体が防ぐはずだった、まさにそのバグを呼び戻します。
