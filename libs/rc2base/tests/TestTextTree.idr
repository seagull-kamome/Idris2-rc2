...
main = do
  ...
  let (_ ** manyparts) = Data.Text.concat (map (\c => (_ ** Data.Text.singleton c)) (unpack "abcdefghijklmnopqrstuvwxyz"))
  ...
  -- directly, do.
  let (_ ** left20) = Data.Text.concat (map (\c => (_ ** Data.Text.singleton c)) (unpack "ABCDEFGHIJKLMNOPQRST"))
  let (_ ** right20) = Data.Text.concat (map (\c => (_ ** Data.Text.singleton c)) (unpack "UVWXYZ0123456789+-*/"))
  ...
  let (_ ** cat) = Data.Text.concat [(n1 ** txt1), (_ ** txt2)]
  ...
  let ((_ ** spanA), (_ ** spanB)) = Data.Text.span (/= ' ') sentence
  putStrLn $ "span (/= ' '): (" ++ toString spanA ++ ", " ++ toString spanB ++ ")"
  let ((_ ** breakA), (_ ** breakB)) = Data.Text.break (== ' ') sentence
  putStrLn $ "break (== ' '): (" ++ toString breakA ++ ", " ++ toString breakB ++ ")"

  let (_ ** csv) = fromString "a,b,,c"
  putStrLn $ "split (== ','): " ++ show (map (\(_ ** s) => toString s) (Data.Text.split (== ',') csv))
