...
main = do
  ...
  traverse_ (\i => Bits8.store arr8 i (cast (finToNat i) + 10)) allFins
  vs8 <- traverse (Bits8.fetch arr8) allFins
  printLn (vs8 == [10, 11, 12, 13])
  printLn (length arr8 == 4)

  arr16 <- newCArray {ty = Bits16} 4
  traverse_ (\i => Bits16.store arr16 i (cast (finToNat i) + 1000)) allFins
  vs16 <- traverse (Bits16.fetch arr16) allFins
  printLn (vs16 == [1000, 1001, 1002, 1003])

  arr32 <- newCArray {ty = Bits32} 4
  traverse_ (\i => Bits32.store arr32 i (cast (finToNat i) + 100000)) allFins
  vs32 <- traverse (Bits32.fetch arr32) allFins
  printLn (vs32 == [100000, 100001, 100002, 100003])

  arr64 <- newCArray {ty = Bits64} 4
  traverse_ (\i => Bits64.store arr64 i (cast (finToNat i) + 0xfeedfacecafebeef)) allFins
  vs64 <- traverse (Bits64.fetch arr64) allFins
  printLn (vs64 == [0xfeedfacecafebeef, 0xfeedfacecafebef0, 0xfeedfacecafebef1, 0xfeedfacecafebef2])

  arri8 <- newCArray {ty = Int8} 4
  traverse_ (\i => Int8.store arri8 i (cast (finToNat i) - 2)) allFins
  vsi8 <- traverse (Int8.fetch arri8) allFins
  printLn (vsi8 == [-2, -1, 0, 1])

  arri16 <- newCArray {ty = Int16} 4
  traverse_ (\i => Int16.store arri16 i (cast (finToNat i) - 12345)) allFins
  vsi16 <- traverse (Int16.fetch arri16) allFins
  printLn (vsi16 == [-12345, -12344, -12343, -12342])

  arri32 <- newCArray {ty = Int32} 4
  traverse_ (\i => Int32.store arri32 i (cast (finToNat i) - 1234567890)) allFins
  vsi32 <- traverse (Int32.fetch arri32) allFins
  printLn (vsi32 == [-1234567890, -1234567889, -1234567888, -1234567887])

  arri64 <- newCArray {ty = Int64} 4
  traverse_ (\i => Int64.store arri64 i (cast (finToNat i) - 9223372036854775808)) allFins
  vsi64 <- traverse (Int64.fetch arri64) allFins
  printLn (vsi64 == [-9223372036854775808, -9223372036854775807, -9223372036854775806, -9223372036854775805])

  arrInt <- newCArray {ty = Int} 4
  traverse_ (\i => System.FFI.C.Array.Int.store arrInt i (cast (finToNat i) + 1234567890123)) allFins
  vsInt <- traverse (System.FFI.C.Array.Int.fetch arrInt) allFins
  printLn (vsInt == [1234567890123, 1234567890124, 1234567890125, 1234567890126])

  arrD <- newCArray {ty = Double} 4
  traverse_ (\i => Double.store arrD i (cast (finToNat i) + 0.5)) allFins
  vsD <- traverse (Double.fetch arrD) allFins
  printLn (vsD == [0.5, 1.5, 2.5, 3.5])

  raw <- malloc 16
  arrP <- newCArray {ty = AnyPtr} 2
  AnyPtr.store arrP FZ raw
  p0 <- AnyPtr.fetch arrP FZ
  Bits32.unsafePtrStore (prim__castPtr {t = Bits8} p0) 0 0xcafef00d
  v <- Bits32.unsafePtrFetch (prim__castPtr {t = Bits8} raw) 0
