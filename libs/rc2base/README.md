...
entry named the fix: rc2's own `cast {to=Double}`/`cast {to=String}`
(`rc2/support/rc2/numeric.c`'s `idris2rc2_parse_double`/
`idris2rc2_shortest_double`) are correct and locale-independent but pay
for GMP bignum temporaries on every call. `fastParse`/`fastShow` are an
**opt-in** drop-in replacement for those two casts specifically (same
total semantics -- unparseable input becomes `0.0`, same
`nan`/`inf`/`-inf`/`-0.0`/shortest-round-trip-decimal output shape) --
not a change to `cast` itself, which stays on the always-correct slow
path unconditionally.

Both directions do the well-known table-driven fast computation
(Eisel-Lemire for parsing, a DiyFp/Grisu2-style scaled digit generation
for formatting) over plain `uint64_t`/`__uint128_t` arithmetic, no GMP
involved -- but wherever that fast computation's own error bound
leaves any doubt, they defer outright to the exact functions above
(exposed non-`static` in `numeric.c`/`numeric.h` for exactly this
reuse) rather than trying to shave the last bit of margin.

The formatter's own correctness doesn't ride on getting its digit
generation exactly right at all -- it can't, in the following sense:
a Grisu2-style loop generates digits from `v`'s upper *boundary*, not
from `v` itself, and the "last digit" it settles on is only ever a
starting guess, occasionally off from the true value by more than one
(confirmed directly during development), and round-tripping alone
can't always catch this (multiple adjacent decimal strings, especially
at extreme magnitudes, routinely parse back to the exact same
`Double`, so "does it round-trip" doesn't imply "is it the specific
digits `idris2rc2_shortest_double` would produce"). So the formatter
never trusts the generated digits on their own: it searches a small
bounded neighborhood (still all `idris2rc2_fastParseDouble` calls, no
GMP) for a round-tripping candidate, then requires it to be the
**unique** one there -- neither immediate neighbor may also
round-trip -- before accepting it. A value with more than one
round-tripping candidate this close (the same extreme-magnitude
scenario above) fails that uniqueness check and falls back outright.
This was arrived at empirically, not designed in from the start: a
5-million-case fuzz run against the exact functions caught first a
window-position bug, then a sign error and a related digit-count bug
in how the scaled decimal exponent was recovered, then this
last-digit-correctness gap itself -- each fix's own reasoning is left
in the code's comments (`fast_shortest_double`/`digit_gen` in
`double_convert.c`) since each documents a real, once-shipped-looking-
correct-but-wasn't failure mode, not just a design choice.

Both lookup tables (`~650`-entry 128-bit powers of five for the parser,
`~87`-entry 64-bit cached powers of ten for the formatter) are
generated by exact GMP computation
(`libs/rc2base/support/c/gen_double_tables.c`, a documented, one-off
tool -- not part of the normal build) rather than hand-copied from a
paper, and checked into `pow5_table.h`/`cached_powers.h`.

`fastShow`'s own native function,
`idris2rc2_fastShowDouble`, builds a fully-formed
`IDRIS2RC2_String` directly (`idris2rc2_mkEmptyString`, the same
one-allocation technique `Data.String.RC2`/`Data.TextBuffer` already
