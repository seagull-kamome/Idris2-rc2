...
char *fastPack(IDRIS2RC2_Value *charList)
    __attribute__((deprecated("leaks its own malloc'd buffer -- Compiler.RC2.Emit's own createCFunctions should always redirect Prelude.Types.fastPack to idris2rc2_fastPackFixed instead, so reaching this is an rc2 bug, not something to work around (see KNOWN-BUGS.md / rc2/doc/fastpack-fix.md)")));
IDRIS2RC2_Value *fastUnpack(char *str);
char *fastConcat(IDRIS2RC2_Value *strList)
    __attribute__((deprecated("leaks its own malloc'd buffer -- Compiler.RC2.Emit's own createCFunctions should always redirect Prelude.Types.fastConcat to idris2rc2_fastConcatFixed instead, so reaching this is an rc2 bug, not something to work around (see KNOWN-BUGS.md / rc2/doc/fastpack-fix.md)")));
