...
1. **共有クロージャへの最終引数適用の割り当て回避**(コミット`49e3f00`/`7f1fcbb`、
   詳細は`rc2/doc/closure-dispatch-optimization.md`)。2026-08-18の「unique
   closure fast path」はクロージャの参照カウントが1(unique)の場合のみ
   再確保を省いていたが、`map`のように**共有**されたクロージャ(参照カウント
   >1)へ**最後の1引数**を適用する場合は、依然として毎回新規`mkClosure`+
   コピー+即トランポリン破棄という無駄な往復が残っていた。共有クロージャの
   既に埋まっている引数を`dup`しつつ、対応する`FUNn`型関数ポインタへ直接
   渡す新規`idris2rc2_dispatchWithExtra`を追加し、この場合に限りクロージャ
   オブジェクトを一切生成せず即ディスパッチするようにした。末尾位置の
   `idris2rc2_tailcallApplyClosure`自体は意図的に変更していない(Cスタックを
   有界に保つトランポリン機構の前提が崩れるため、`idris2rc2_applyClosure`
   側だけの対応)。
2. **定数クロージャの畳み込み**(コミット`a01eaa2`/`c402f37`/`0e7c755`/
   `c46d1e1`、詳細は`rc2/doc/const-closure-fold.md`)。「引数未適用の
   名前付きトップレベル関数へのクロージャ参照」(`RUnderApp fc n missing []`)
   を`Compiler.RC2.ConstFold`の新規`RCConstClosure`定数として認識できる
   ようにし、既存の`RCConstCon`畳み込み機構にそのまま乗せた。インター
   フェース辞書(メソッドごとにクロージャを持つレコード)のような、
   フィールド全部が此の形のコンストラクタは、毎回の呼び出しで割り当て
   直されていたのが、コンパイル時に1個の不変staticへ完全に畳み込まれる
   ようになった。`{__mainExpression:0}`自身の継続クロージャも同じ形なので、
   この畳み込みは**インターフェースを使わないプログラムでも常に発火する**。

### マイクロベンチマーク一式(`rc2/tests/bench.sh`、rc2 vs 本家`idris2 --cg refc`、壁時計3回平均)

| ベンチマーク | rc2(s) | refc(s) | 倍率(RefC比) |
|---|---|---|---|
| `BenchChain.idr` | 0.008 | 0.655 | **81.9倍高速** |
| `BenchLoopCallArg.idr` | 0.008 | 0.533 | **66.7倍高速** |
| `BenchLoop.idr` | 0.003 | 0.177 | **58.9倍高速** |
| `BenchMutual.idr` | 0.015 | 0.182 | 12.1倍高速 |
| `BenchConstConFold.idr` | 0.361 | 1.404 | 3.9倍高速 |
| `BenchCallArgChain.idr` | 0.867 | 1.577 | 1.8倍高速 |
| `BenchClosureChain.idr` | 0.453 | 0.765 | 1.7倍高速 |
| `BenchFib.idr` | 0.127 | 0.203 | 1.6倍高速 |
| `BenchLoopInvariantBoxed.idr` | 0.227 | 0.315 | 1.4倍高速 |
| `BenchTailFFI.idr` | 0.988 | 1.332 | 1.35倍高速 |
| `BenchConAltNativeBoxed.idr` | 0.251 | 0.247 | 約1.0倍(ほぼ同等) |

いずれも既存ベンチマークの絶対値で、今回追加した2件専用の新規マイクロ
ベンチマークは作成していない(正当性は`Test66`〜`Test73`の生成C直接確認と
valgrindで担保、上記「共有クロージャ...」節参照)。全件既存の`Bench*.idr`
に対する回帰は無し。

### 外部パッケージベンチマーク(idris2-missing-containers)再計測

同一セッション内で、今回の2件の変更**前後**を直接比較した(壁時計3回平均、
辞書ベンチマークの実行対象・手順は下記「外部パッケージベンチマーク」節を
参照):

| | 直前(共有クロージャ最適化後・定数畳み込み前) | 今回(定数畳み込み後) |
|---|---|---|
| rc2 | 14.54s | **12.76s** |
| RefC | 20.54s | 20.44s |
| Chez | (未計測) | 7.03s |

定数クロージャ畳み込みだけで**約12%の追加短縮**(14.54s→12.76s)。RefC/Chez
はこの変更の対象外なので、この前後比較はマシン負荷等のセッション間ノイズを
含まない、同一実行環境内での純粋な差分である。

一方、絶対値を過去の記録(2026-08-18「クロージャ部分適用チェーンのin-place
伸長」時点、10.92s)と比べると悪化して見える(10.92s→12.76s)。ただし
RefC側の記録値も21.91s→20.49s→20.44sとセッションを跨いで変動しており、
この節が既に述べている通り絶対値のセッション間比較はマシン負荷ノイズの
影響を受けやすい。rc2/RefC比で見ても2026-08-18時点の約1.88倍から今回は
約1.60倍に縮んでおり、比率まで含めた低下は絶対値だけの変動よりは説明が
つきにくく、素直に「悪化しているように見える」という計測結果として記録
しておく——ただし今回はフェーズ別の再計測は行っておらず、2026-08-18から
今回までの間(自己末尾再帰ループのネイティブ表現化拡張・`%export`拡張・
テストスイート再編等、多数の無関係な変更を含む)のどこかで実際に何らかの
退行が入ったのか、単に今回の計測環境(同日中の別ベンチマーク・ビルド
作業が同一マシン上で連続して行われていた)によるノイズなのかは切り分け
られていない。同一セッション内の前後比較(今回の2件の変更の純粋な効果、
上表)は信頼できるが、2026-08-18からの絶対的な退行の有無は、機会があれば
クリーンな環境で改めて検証する価値がある。
...
| | OFF | ON |
|---|---|---|
| `apply` | 11,384 | **11,182** (−202) |
| DeadCode後の定義数 | 26,387 | **25,978** (−409) |
| `reuse=`(再利用成功) | 21,829 | **22,195** (+366) |
| Nativeな`RLet` | 3,349 | 3,422 (+73) |
| IR行数 | 732,238 | 735,646 (+0.5%) |
| `dup` / `drop` | 93,645 / 77,223 | 94,165 / 77,813 (+0.6%) |
| コンパイル時間(3回の中央値) | 27.49s | 28.59s (+4.0%) |

複製を足しているのに定義数が**減る**のは、呼び出し元を失った元定義を
`DeadCode`が複製より多く刈るため。

キー1,598件の内訳は`--directive timing`で出る。695件がscrutinee-onlyゲートを
通り、578件が採算ゲートで採用され、最終IRに73件が残る。残り903件(57%)は辞書が
**別のcallee**へ渡される形で、関数間の定数伝播が必要なため対象外。

### 実行時A/B(同一マシン5回、各々`--directive nospecconstcon`と対比)

| | OFF | ON | |
|---|---|---|---|
| `tests/BenchSpecConstCon.idr` | 0.48s | **0.37s** | 約23%高速 |
| `tests/BenchSpecConstConRec.idr` | 0.49s | **0.40s** | 約18%高速 |

生成C中の`idris2rc2_applyClosure`はいずれも 4 → **0**。出力は全て`937501`で一致。
前者は非再帰のcalleeに辞書が届く形、後者は辞書を**自分の再帰に持ち回る**形で、
ゲートの両半分をそれぞれ担当する。

idris2-lsp全体で`apply`が9件しか減らないのは自己持ち回り許容分の**静的な**値で
あって、実際にはそれはループの中の dispatch —— ノードごとではなく反復ごとに
払っていたコストである。実コードで普通に現れる形はむしろこちら。
...
### 分岐の末尾への`case`の押し込み(`Compiler.RC2.PushCon`、書き換えB)

`let v = case ..; case v of`のうち、内側の分岐が**コンストラクタで終わる**形
(`Core`の`do`ブロックのbind連鎖)で、外側の`case`を内側の各末尾へ押し込む。
末尾のコンストラクタはそこで自分の分岐とだけ出会い、書き換えAで畳まれる。

idris2-lsp全体(`--directive nopushcon`との対比、書き換えAは両方で有効):

| | OFF | ON |
|---|---|---|
| 対象の形 | 2,327 | **1,625** (−702) |
| 最終IRの`con` | 59,570 | 60,475 (+905) |
| IR行数 | 717,609 | 720,428 (+0.4%) |
| 段の所要時間 | — | 0.22s |

`con`が静的に**増える**のは、小さい分岐(`Left e`の包み直し)を末尾ごとに
複製するため。1回の実行で通るのはそのうち1つで、置き換えたコンストラクタは
その経路ではもう作られない。

実行時A/B(`tests/BenchPushCon.idr`):

| | OFF | ON | |
|---|---|---|---|
| 壁時計(500万回、5回) | 3.54s | **3.45s** | 約2.6%高速 |
| valgrindの確保回数(10万回) | 1,999,977 | **1,899,992** | 1回の呼び出しにつき1回減 |

出力はいずれも`985393`で一致。減った確保は、連鎖の最終結果`Right (a + b)`の
ネイティブな和をフィールドに入れるためのボックス。コンストラクタのセル自体は
もともと`Reuse`が使い回していたので、確保回数には効いていない。
LateInlineが後から作る形(このベンチマークでは新規確保の末尾)にはこの段が届かず、残る
1,625件の約半分がそれにあたる。

### 単一呼び出し元の早期インライン化 + 部分適用の畳み込み

`Compiler.RC2.Inline`のCriterion B(ループを含まない単一呼び出し元の関数を
`Lifted`段階で展開)と、`ConstFold`の部分適用畳み込み(逃げない
`partial f`をちょうど足りない引数で`apply`する形を直接呼び出しにする)。
`doc/inlining.md`の「Criterion B at `Lifted`」と
`doc/constructor-escape-analysis.md`を参照。

idris2-lsp全体(この変更の前後):

| | 前 | 後 |
|---|---|---|
| `apply` | 11,326 | **10,039** (−11%) |
| `partial` | 16,448 | **15,788** |
| 確保する`con`(`reuse=`なし) | 39,741 | **38,257** |
| `dup` / `drop` | 92,764 / 76,253 | 92,136 / 75,857 |
| IR行数 | 720,428 | 711,725 (−1.2%) |
| コンパイル時間(3回の中央値) | 27.9s | 29.3s (+5%) |

`tests/BenchPushCon.idr`(`check1`/`check2`が早期に展開され、そのコンストラクタも
押し込んだ`case`と出会って畳まれる):

| | `nopushcon` | 全て有効 |
|---|---|---|
| valgrindの確保回数(10万回) | 1,999,976 | **1,500,052** |
| 壁時計(500万回、5回) | 3.42s | **2.80s**(約18%高速) |

この変更の前は、同じベンチマークが全て有効で3.45sだった。

### RC注釈前の単一呼び出し元展開(Early inline)

SpecConstCon の後、RC 注釈の前に、`LateInline` の単一呼び出し元展開をもう一度
走らせ、結果を ConstFold と PushCon で畳み直す段。CAF と呼び出しの輪に入っている
関数は対象外。`doc/constructor-escape-analysis.md`の「Early inline」を参照。

idris2-lsp全体(`--directive noearlyinline`との対比):

| | OFF | ON |
|---|---|---|
| `partial`(クロージャ生成) | 15,790 | **11,115** (−30%) |
| 確保する`con` | 38,241 | **34,916** (−9%) |
| `dup`増分 / `drop`減分 | 105,628 / 201,069 | **92,240 / 188,058** |
| 定義数 | 25,550 | 22,522 |
| 書き換えAの形 / 書き換えBの形 | 308 / 1,785 | **72 / 1,189** |
| コンパイル時間(交互に3回ずつ) | 33.8s | 36.9s (+9%) |

計測中は他の負荷があった(load average 約2)ため、絶対値は以前の計測より大きいが、
...
| | 無効 | 有効 | |
|---|---|---|---|
| 壁時計 | 674ms | **475ms** | 約30%高速 |
| valgrindの確保回数 | 10,000,042 | **5,000,042** | 1回の呼び出しにつき1回減 |

出力はいずれも`177500000`で一致。有効時に残る1回の確保は`mod x 64`が
演算対象を箱詰めする分で、構造体返しとは無関係。

idris2-lsp全体(最終IR、C生成までは届かない):

| | 無効 | 有効 |
|---|---|---|
| 定義数 | 22,522 | 29,609 (worker +7,087) |
| `con` | 53,957 | **49,167** (−8.9%) |
| `retpack` | 0 | 17,643 |
| `reuseOffer` | 25,765 | **17,865** (−31%) |

idris2-missing-containers(`test/src/Main.idr`、交互に5回、1回目は温まりとして除外):

| | 無効 | 有効 |
|---|---|---|
| 壁時計(平均) | 12.13s | 12.29s (+1.3%) |

効果なし。対象になる関数は5つだけで、どれもクロージャとして渡されるラムダ
(`{read:0}`など)のため、呼び出しは必ず`apply`経由でwrapperに届き、
呼び出し1段とセルの組み立て直しが余分にかかる。

その後、得をする呼び出し元(結果をすぐ`case`する呼び出し、または構造体を返す
workerからの末尾呼び出し)がある関数だけにworkerを作るよう絞り込んだ
(`prunePlan`)。missing-containersでは対象が0件になり、無効時と同じ
...
### ネイティブ値のフィールド(2026-09-26)

構造体の`f0`を`{p, i, d}`の共用体にし、あるタグのフィールドを作る末尾が
すべて同じ固定幅の整数・`Char`・`Double`を渡すときは、そのまま載せる
(ダンプでは`Ret1:1=Int`)。

`tests/BenchStructReturnNative.idr`(`BenchStructReturn`で値を全て100以上に
したもの、500万回、5回の最良値):

| | 構造体返しなし | 構造体返しあり |
|---|---|---|
| 壁時計 | 1,723ms | **864ms**(約50%短縮) |
| valgrindの確保回数 | 50,000,138 | **20,000,138** |

出力はいずれも`12505037500000`で一致。workerは確保を一度もしない。残る
1回あたり4回の確保は、ループの`i + 1000`がリテラル`1000`を毎回`Integer`で
作って`Int`へ変換する分で、構造体返しとは無関係(TODOに記録)。
idris2-lspでは効く箇所がほとんどない(`retpack` 3,679件のうちネイティブの
局所変数5件、整数リテラル73件)。
...
## 構造体返しを4フィールドまで拡張(2026-09-26)

構造体返しの対象を、2〜4フィールドのコンストラクタを返す関数にも広げた
(`IDRIS2RC2_Ret1`〜`IDRIS2RC2_Ret4`、`doc/struct-return.md`の
"Wider constructors")。

idris2-missing-containers(交互に6回、1回目を除いた平均)では、ハッシュ関数
3つ(SipHashの`round64`/`round32`、MurMur3の`feed8_tick`)が対象になる。
どれもバイトごとにタプルを返し、呼び出し元がすぐに分解する。1フィールド版
では対象0件だったので、構造体返しなしと比べる:

| | 構造体返しなし | 4フィールドまで |
|---|---|---|
| 壁時計 | 9.63s | **8.15s**(−15%) |
| MurMur3 | 311ms | 230ms(−26%) |
| Sip64 | 474ms | 405ms(−15%) |
| Sip32 | 473ms | 361ms(−24%) |
| write(ハッシュマップへの挿入) | 6,483ms | 5,394ms(−17%) |
| read(ハッシュマップの検索) | 646ms | 535ms(−17%) |
