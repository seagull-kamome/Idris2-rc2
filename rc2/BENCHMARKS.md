...
2. **rc2とは無関係な既存の問題(未解決、対応スコープ外)**: 上記の
   修正後、`benchmarkHashMap`実行中に`Unhandled input for Main.case
   block`という**実行時**エラーに遭遇した。念のためデュアルABI導入前の
   コミット(以前の15.20秒という結果を出した、まさにそのコミット)まで
   遡ってbisectしたが、**全く同じエラーが再現**した。さらに念のため
   本家の無改造`idris2 --cg refc`でも同じワークロードを試したところ、
   **これも全く同じエラー**で失敗することを確認した。つまりこれは
   rc2・デュアルABIとは完全に無関係な、パッケージ側または環境側の
   既存の問題であり(`Test7CastMatrix.idr`のcontribパッケージ解決問題と
   同種の、環境ドリフトによるものと推測される)、今回の作業のスコープ外
   と判断した。この問題により、外部パッケージベンチマークのデュアルABI
   導入後の再計測は現時点で行えていない。

**結論**: デュアルABI自体に起因する新たなバグはこの過程で見つからなかった
(1つ目のバグはStage 3aから存在した既存のバグで、Stage 4のbisect時に
たまたま発見されただけ)。外部パッケージでの実測値更新は、上記の
既存問題が解消されるまで持ち越しとする。
