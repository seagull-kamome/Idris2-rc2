...
| `noknowncon` | Only `Compiler.RC2.ConstFold`'s known-constructor fold -- resolving a `case` on a non-escaping constructor built in the same function, which drops the construction (`doc/constructor-escape-analysis.md`, "Rewrite A"). The rest of `ConstFold` still runs; implied by `noconstfold`. The clones `Compiler.RC2.SpecClosure` folds itself keep the fold. |
