...
上記の形状制限は*何が*畳み込まれるかを制限しているのであって、
*どれだけ*かは制限していない -- `doCaseOfCase`/`doCaseOfConstCase`は
内側の枝1つにつき外側の`alts`/`def`丸ごとを1回複製するが、`alts`/
`def`自身のサイズには何の上限も無かった。小さく、caseを返す、呼び
出しを含まない関数の連鎖(まさに基準A自身の対象形状 -- 例えば
`if pI x then ... else ...`というガードの連続で、各`pI`が小さな列挙
駆動の述語であるようなもの)が連続するスクルティニー位置へ差し込ま
れると、これは*乗算的に*積み重なる: 各段が、その下の段で既に畳み
込まれた結果を、自分自身の枝の全てへ複製するからである。

これは、同じ`CaseOpts`コードに対する本家自身の露出のしかたとは
アーキテクチャ上異なる(上記「Case-of-caseの畳み込み」参照): 本家の
既定の自動インライン化ヒューリスティック(`Compiler.Opts.
InlineHeuristics`の`simple`)は、本体自身が`CConCase`/`CConstCase`
であるcalleeを明示的に除外するので、本家自身の`caseOfCase`はソース
に元々存在していたネストしか一切畳み込まず、自分自身のインライナが
たった今作ったばかりのネストを畳み込むことは決してない。このパスは
意図的にその逆の形を狙っている -- 小さくcaseを返すcalleeをスクル
ティニー位置へインライン化することそのものが目的の全て(上記「動機」
参照)なので、本家の「case形の本体はインライン化しない」という
ガードを、自分自身の存在理由を失わずに採用することはできない --
本家のアプローチは本当にここへは持ち込めない。

合成したNレベルのガード連鎖(`if pI x then ... else ...`、
`pI : Tri -> Bool`は3値の列挙述語 -- `rc2/doc/inlining.md`自身の
履歴。生成スクリプトはコミットされていない)で実証的に確認した:
生成されるCの行数は追加の1段ごとにおよそ*倍増*した -- N=5/10/11/12
でそれぞれ1,185/19,041/37,473/74,337行 -- 同じN=10のソースを
`--directive noinline`でコンパイルした場合の1,044行(Nと共に一切
増えない。そもそもスクルティニー位置へ何も差し込まれないため)と
対照的である。N=15あたりでメモリを完全に使い果たした。重要な点
として、`--timing 2`は、肥大化した木を構築している最中でも
`rc2: Inline`自身は~0msで終わっていることを示した(既に構築済みの
ノードを複製するのは安価なアロケーションでしかない); 実際の
wall-clockコストが可視化されるのは、この今や巨大になった木に対して
ノードごとに本物の作業を行う*次*の段階(`rc2: RC normalize`、N=10で
0.378秒)になってからである -- そのため、wall-clockやメモリの観測
から「インラインステージ」の問題だと帰属されるハングや遅延は、
サイズの爆発そのものはそこに由来していても、`Compiler.RC2.Inline`
自身の`logTime`行より*後段*に現れうる。

**修正**: `tryCaseOfCase`の2つの節は、今や
`duplicationCount * outerSize <= caseOfCaseSizeBudget`
(`caseOfCaseSizeBudget = 200`)も要求するようになった。ここで
`outerSize`は外側の`alts`/`def`に適用した`sizeOf`/`sizeOfConAlt`/
`sizeOfConstAlt`(基準A自身の`smallBodyThreshold`が使うのと同じ、
大まかな構造ノード数 -- このガードが再利用できるようファイルの
より前方へ移動した)であり、`duplicationCount`はそれらが複製されて
入る箇所の数(`length xalts`、`xdef`があればさらに1つ加える)である。
畳み込みをスキップすることは常に意味論的に安全である -- 結果は単に
元の、畳み込まれていない`case (case ...) of ...`のままで、正しいが
そこから先は融合されない、というだけである。このチェックは`alts`/
`def`の*既に実現済みの*サイズ(木のより下側での畳み込みによる複製が
既に反映されている)についてボトムアップで走るので、さもなければ
積み重なり続けたはずの連鎖は、累積サイズが予算を超えた最初の段で
キャップされる; それより上の全ての段は、既に予算に達したか超過した
入力を目にし続けるので、乗算が再開することはなくスキップし続ける。

修正後に同じ合成ジェネレータを再実行したところ: 生成されるCの行数は
代わりにおよそ*線形*に増加した -- N=5/10/12/15/20でそれぞれ
729/822/862/922/1,022行(追加の1段あたり約20行) -- そしてフルの
回帰スイート(87/87、`Test15CompareFusionThroughCall`の機能面・
`valgrind`面の両方を含む)は緑のまま維持され、この予算が、このパス
自身の動機となったケースにおいて実際に発生するサイズでは干渉しない
ことを確認した。

### サイズの記帳: 持ち回し、再走査ではなく

上記の合成Nチェーンのベンチマークはもうクラッシュしなくなったが、
それはガード自身の*計測*コストを一度も運動させていなかった: *あら
ゆる*候補箇所で`alts`/`def`に対して新たにトップダウンの`sizeOf`/
`sizeOfConAlt`走査を行って`outerSize`を計算するコストである。大きな
実プログラムでは、この走査コストが`rc2: Inline`自身のwall-clock時間
を完全に支配してしまった -- ある実際の大きなプログラムで144秒。
畳み込みの*複製*自体は、上記の予算によって既に正しく制限されていた
にもかかわらずである。決め手となった兆候: 直後の`rc2: RC normalize`
は、同じプログラムで`--directive noinline`と既定設定の間に計測可能
な差が全く無かった(0.20秒 対 0.24秒)。つまりコストは`rc2: Inline`
自身の*内側*にあり、その後段ではなかったということである(先の合成
Nチェーンのケースとは異なる。あちらではサイズの爆発は本物だったが、
Inline自身の`collapseCaseOfCase`は速く終わり、今や大きくなった木を
処理する可視のwall-clockコストは`rc2: RC normalize`が吸収していた)。
大きな実プログラムには、散在するcase-of-caseの候補箇所が多数ある
(病的な1本の連鎖だけではなく)。その1つ1つが、自分を囲む、上限の
無い大きさの`alts`/`def`をゼロから再走査する -- 実質的にO(候補箇所
の数 × 平均的な周囲の文脈サイズ)である。

**修正**: `collapseCaseOfCase`/`collapseConAlt`/`collapseConstAlt`は、
今や裸の木ではなく`Sized`ペア(`szOf`/`valOf`)を返すようになった。
これにより各ノード自身のサイズは、それを組み立てていたのと同じ
ボトムアップの畳み込み(fold)の副産物として、増分的にちょうど1回だけ
計算される -- 別立ての走査で再導出されることは一切無い。
`caseOfCaseHere`の再試行ループ(木の1箇所につき最大5回試行)は、裸の
木ではなく`CollapseState`(`totalSize`、`branchesSize` -- すなわち
`outerSize` -- と木自身)を持ち回すので、1箇所での連続した畳み込みの
*連鎖*も一切再走査しない: `doCaseOfCase`/`doCaseOfConstCase`は、
結果から再導出する代わりに、閉じた式で両方のサイズを更新する --

```
newBranchesSize = sizeOf(xalts) + sizeOf(xdef) + duplicationCount * (1 + outerSize)
```

(`alts`/`def`の複製をより深いスコープへ再インデックスするために使う
`weakenNs`は、ノード数を一切変えないので、複製されたコピーのサイズ
は常に入力の`outerSize`とぴったり一致する; `duplicationCount`個の
コピーそれぞれは、`updateAlt`/`updateDef`が周りに組み立てるラッパー
ノード1個分も追加で得るので、コピーごとに`+ 1`となる。)`xalts`/
`xdef`(*内側*の、たった今差し込まれたばかりの側)は、依然として素の、
持ち回されていない`sizeOf`/`sizeOfConAlt`/`sizeOfConstAlt`で新たに
走査される -- それらがこのパス自身のインライン化に由来する場合は、
プログラムのサイズに関係なく基準A自身の`smallBodyThreshold`で上限が
付いているし、代わりに本当に大きく、元のソースに自然に存在していた
ネストした`case`(このパスの`collapseCaseOfCase`はインライン化された
かどうかにかかわらず*あらゆる*定義に対して走る)に由来する場合は、
その走査はこの記帳の仕組みが存在する前から既にちょうどこれだけの
コストがかかっていた -- そちら側には新しいコストは一切導入していない。

再検証済み: 同じ合成ジェネレータで、上記と同じ線形の行数増加(サイズ
を持ち回すのは純粋な性能上の変更であって、挙動上の変更ではない)、
そしてフルの回帰スイートは緑のまま維持された(87/87、
`Test15CompareFusionThroughCall`込み)。

これを追いかけている最中に、別件の無関係なコストも見つかった:
同じ合成ジェネレータのN=25では、本家自身の「Elaborating」ステップで
5.6秒かかった(rc2とは無関係 -- `rc2:`接頭辞の付く段は全て0.000秒の
ままだった) -- 文字通り26段ネストした`if`-`then`-`else`式を1個
elaborateすることは、`--directive noinline`の有無にかかわらず、
本家自身のelaboratorにとってどうやら高コストらしい。これ以上は
追跡していない(実際のソースコードが、この深さの1行N段`if`連鎖を
手で書くことは実質的に無い; 合成ジェネレータはcase-of-caseの形を
切り分けるためだけに意図的にそうした)が、再発した際にこのパス自身
の挙動と混同されないよう記録しておく価値はある。
