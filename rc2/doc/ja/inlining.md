...
書かれていた: `findLoopThroughLets`は純粋な`RLet`の連なりの先に
到達可能な*最初の*`RLoop`しか見つけないし、`Loop.idr`自身の
`nativeArgTypesFor`/`nativeArgTypes`(そういう連なりの先にループが
無い場合に`paramEligibility`が使うフォールバック)には`RLoop`の
ケース自体が無い -- どちらも、`Compiler.RC2.Loop`自身(当時のその
唯一の生成元)以外が`RLoop`を含む本体を目にすることが無かった間は
正しかった。

このパスが、既に自分自身のループを持っているかもしれない呼び出し元
へLoop変換済みのcalleeを差し込めるようになった今(上記「1関数内の
複数ループ」参照)、インライン化されたループの*内部*だけでネイティブ
に使われているトップレベルパラメータは、`paramEligibility`のどちらの
経路からも見えなくなり、ネイティブworker引数へ昇格されることが無い。
これは仮説ではなく実際に確認された穴である(`DualABI.idr`/`Loop.idr`
を直接読んで追跡した) -- このパス自身の差し込み方法に固有の問題では
なく、別の、既にデリケートなモジュールにあった既存の前提なので、
このパスの中で対処するのではなく`findLoopThroughLets`/
`nativeArgTypesFor`への追加修正として追跡している。
