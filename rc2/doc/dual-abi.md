...
- **No wrapper rewrite -- the original `MkRCForeign` is untouched.**
  ...
  (`emitFFIWorker`) alongside the always-emitted wrapper -- which
  itself is generated by exactly the same code as before this stage
  existed, unconditionally, byte-for-byte.
- **Stage 4 needed zero changes.** A `%foreign`-declared name's own
  ...
  *caller* side) -- `applyCallSiteRewriteBody`'s existing rewrite rule
  ...
  regardless of whether that entry came from Stage 3a or 3c.
  `applyCallSiteRewrite` just unions `ffiWorkerTable`'s own result into
  the `workerTable` it already builds from scanning `MkRCFun` wrappers
  (`mergeWith const` -- the two key sets are always disjoint, a name is
  never both a `MkRCFun` and a `MkRCForeign`).
...
would be. `rc2/tests/Test27FFIDualABI.idr` is the regression/smoke
test (Int/Bits64/Double all-native args+return, a mixed Int+String
signature, and an Int arg with a Boxed `CFUnit` IO return); confirmed
correct against real `idris2 --cg refc` byte-for-byte, leak-free
(`valgrind --leak-check=full`, `definitely lost: 0 bytes`), and a
measured win on that test's own 200000-iteration loop (`--directive
nodualabi` A/B, `time`, three runs each): ~0.055s with the FFI worker
enabled vs. ~0.067s without, roughly **18% faster** -- smaller than
`fib`'s own 35% since this loop's own body does real non-FFI work
too (large-integer-literal casts), not purely FFI-call-bound.
...
**Fully implemented and verified** (Stages 1, 2, 3a, 3b, 3c, 4).

Stage 3c (FFI worker synthesis): `rc2/tests/Test27FFIDualABI.idr`'s own
`prim__add` compiles to a wrapper (`Main_prim__add`, unchanged Boxed
signature, `extractValue`/`packCFType` untouched) plus a worker
(`idris2rc2_ffiworker_Main_prim__add_4`) taking/returning raw
`int64_t` directly, with the test's own self-tail-recursive `loop`
(already a native `int64_t` goto-loop via `Compiler.RC2.Loop`) calling
that worker straight from inside its own body -- no boxing, no
unboxing at that call at all. Confirmed correct against real `idris2
--cg refc` byte-for-byte, leak-free (`valgrind --leak-check=full`,
`definitely lost: 0 bytes`), the full refc-suite (19/19) and entire
smoke-test matrix still passing, and a measured win on that test's own
200000-iteration loop (`--directive nodualabi` A/B): ~0.055s with the
FFI worker vs. ~0.067s without, ~18% faster.
...
- `rc2/src/Compiler/RC2/DualABI.idr` -- `paramEligibility`/
  ...
  `"idris2rc2_ffiworker_"` share the one collision-checked namer),
  ...
  Stage 3c's own table as an explicit argument and `mergeWith const`s
  it into the `MkRCFun`-derived `workerTable`, unchanged otherwise).
...
- `rc2/src/Compiler/RC2/Emit.idr` -- `createCFunctions` now `Rep`-aware
  ...
  `Compiler.RC2.DualABI`'s own worker-naming reuse; new `FFIWorkers`
...
- `rc2/src/Compiler/RC2/RC2.idr` -- `toRCDefs`'s own pipeline wiring
  ...
  `--directive dumpdualabi` wiring; `toRCDefs` now also returns Stage
  3c's own `ffiWorkerTable` result alongside the `RCDef` list, threaded
  through `compileExpr` into `generateCSourceFile`'s new leading
  argument (seeds `Emit`'s own `FFIWorkers` ref).
...
- `rc2/tests/Test27FFIDualABI.idr`/`.c`/`.h` -- Stage 3c's own
  regression/smoke test; verify with `valgrind --leak-check=full`, same
  reasoning as `Test11DualABILeak.idr` above.
