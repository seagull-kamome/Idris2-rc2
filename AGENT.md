...
- ドキュメント更新（README.md、TODO.md、rc2/doc/\*.md等）は実装作業とは
  別の独立したサブエージェントに任せる。1回のサブエージェントに実装と
  ドキュメント更新をまとめて詰め込まない。
- ドキュメント更新用のサブエージェントは、会話履歴の引き継ぎが不要なら
  fork ではなく general-purpose タイプを使い、引き継ぎコストを避ける。
- テスト実行ログ・ビルドログ・valgrind出力等はサブエージェント自身の
  作業内で確認させ、メインセッションへの報告は成否件数等のサマリーのみ
  とする。生ログ全文を報告に含めさせない。
- サブエージェントも思考は英語で行う。メインセッションへの報告はユーザへ
  提示されるので日本語で応答する。
